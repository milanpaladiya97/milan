		...
		if ( $is_table_exists ) {
			...
			if ( count( $forms ) > 0 ) {
				...
				foreach ( $forms as $form ) {
					...
					$last_es_af_name       = $form['es_af_last_name'];
					$last_es_af_name_mand  = $form['es_af_last_name_mand'];
					...
					$body = array(
						...
						array(
							...
							'params' => array(
								'label'    => 'Last Name',
								'show'     => ( $last_es_af_name === 'YES' ) ? true : false,
								'required' => ( $last_es_af_name_mand === 'YES' ) ? true : false
							),
